...
class EnCost_TrajLearner():
    ...
    def train(self, data):
        ...
        self.eioc_mdl.passive_dyn_ctrl = np.eye(self.n_phases)
        #build data
        train_data = self.build_eioc_traj_data_(data)
        #fit with the eioc model
        if self.verbose:
            print 'fitting...'
        self.eioc_mdl.fit(train_data)
        #extract desired information...
        if self.verbose:
            print 'populating learner parameters...'
        self.populate_learner_parms()
    ...
    def build_eioc_traj_data_(self, data):
        ...
        if self.passive_type is None:
            ...
            res_data = np.array([traj.T.flatten() for traj in data])
        else:
            passive_data = [self.construct_passive_state_(traj, self.passive_type).T.flatten() for traj in data]
            #now concatenate them...
            res_data = np.array([np.concatenate([passive_traj, traj.T.flatten()]) for passive_traj, traj in zip(passive_data, data)])
    ...
    def populate_learner_parms(self):
        """
        ...
        for mode_idx, (mean_traj, traj_covar, traj_cost_const, weights) in enumerate(zip(    self.eioc_mdl.estimators_full_['means'],
            ...
            if self.passive_type is not None:
                ...
                self.mean_trajs.append(np.reshape(mean_traj, (self.n_dofs, self.n_phases)))
                #extract correlated terms to construct the variance for each phase step
            ...
            self.tracking_mats.append([np.linalg.pinv(mat) for mat in tmp_covar_mats])
            #for the constant term, simply copy it...
            self.cost_consts.append(traj_cost_const)
            print weights
...
def EnCost_TrajLearner_Test():
    ...
    traj_learner = EnCost_TrajLearner(n_estimators=5, verbose=True)
    traj_learner.train(data=data)

    #see the mean trajs
    transp = traj_learner.mode_weights / np.amax(traj_learner.mode_weights)
